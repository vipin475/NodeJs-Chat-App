...
io.on('connection', (socket) => {
    ...
    socket.on('sendFile', (imageD, callback) => {
        const user = getUser(socket.id)

        const i = imageD.imgData.indexOf('base64,')
        const buffer = Buffer.from(imageD.imgData.slice(i + 7), 'base64')
        const name = `${Math.random().toString(36).slice(-5)}.png`
        const file = new File({buffer, name, type: 'image/png'})

        fs.writeFileSync(path.join(publicDirectoryPath, '/uploadedImages/'+name), buffer, 'utf8');

        io.to(user.room).emit('imageMessage', generateImageMessage(user.username, user.color, name, user.avatar))
        callback()
    })
